...
export class CacheService {
   ...
   * Cache pricing data
   */
  async cachePricingData(provider: string, region: string, data: any): Promise<void> {
   ...
   */
  getCachedPricing(provider: string, region: string): any {
    const key = `pricing:${provider}:${region}`
    return this.get(key)
  }
   ...
   * Cache insights data
   */
  cacheInsights(websiteId: string, insightType: string, data: any): void {
   ...
   * Get cached insights
   */
  getCachedInsights(websiteId: string, insightType: string): any {
    const key = `insights:${websiteId}:${insightType}`
    return this.get(key)
  }
   ...
   */
  cacheRecommendations(websiteId: string, userGoal: string, data: any): void {
   ...
   */
  getCachedRecommendations(websiteId: string, userGoal: string): any {
    const key = `recommendations:${websiteId}:${userGoal}`
    return this.get(key)
  }
   ...
   */
  private calculateMonthlyAverage(data: any[]): number {
  ...
  private aggregateByProvider(data: any[]): Record<string, any> {
  ...
  private aggregateByCategory(data: any[]): Record<string, any> {
    return data.reduce((acc, item) => {
